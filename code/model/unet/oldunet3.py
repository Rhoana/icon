...
bad1 = theano.shared( 999., 'bad1')
bad2 = theano.shared( 998., 'bad2')
bad3 = theano.shared( 997., 'bad3')
bad4 = theano.shared( 996., 'bad4')
bad5 = theano.shared( 995., 'bad5')
...
class UNET(object):
    ...
    def __init__(
        ...
        self.learning_rate = 0.00001#learning_rate
    ...
    def train(self, offline=False, data=None, mean=None, std=None):
        ...
        '''
        data = gen_data(project, 'validation', val_samples, patchSize, patchSize_out)
        ...
        purpose = 'train'
        #futureData = pool.apply_async(stupid_map_wrapper, [[gen_data,project,purpose, train_samples, patchSize, patchSize_out]])
    ...
    @staticmethod
    def aunet_crossentropy_loss_sampled(y_true, y_pred):
        epsilon = 1.0e-4
        ...
        n_samples = T.cast(T.min([ pos.shape[0], neg.shape[0]]), dtype='int64')
        indPos = indPos[:n_samples]
        indNeg = indNeg[:n_samples]

        loss_vector = -T.mean(T.log(y_pred_clipped[indPos])) - T.mean(T.log(1-y_pred_clipped[indNeg])).eval()
        average_loss = T.mean(loss_vector)
        return average_loss
    ...
    def unet_crossentropy_loss_sampled(y_true, y_pred):
        ...
        l1 = T.mean( T.log(y_pred_clipped[indPos]))
        l2 = T.mean( T.log(y_pred_clipped[indNeg]))
        if T.isnan(l1):
            return -T.mean(y_pred_clipped[indPos])

        if T.isnan(l2):
            return -T.mean(y_pred_clipped[indNeg])

        loss_vector = -T.mean(T.log(y_pred_clipped[indPos])) - T.mean(T.log(y_pred_clipped[indNeg]))
        if T.isnan(loss_vector):
            return -T.mean(y_pred_clipped[indPos])
